...
function Card({
  ...
  const bgConfig = customBackgrounds?.[index];

  const bgStyle = (() => {
    if (bgConfig?.type === 'color') return { backgroundColor: bgConfig.value };
    if (bgConfig?.type === 'image') return {
      backgroundImage: `url(${bgConfig.value})`,
      backgroundSize: 'cover',
      backgroundPosition: 'center'
    };
    if (bgConfig?.type === 'video') return { backgroundColor: '#000' };
    if (bgConfig?.type === 'gradient') return { backgroundImage: bgConfig.value };
  ...
  const animationClass = (() => {
    ...
    switch (bgConfig.direction) {
      case 'vertical': return 'animate-gradient-y';
      case 'diagonal': return 'animate-gradient-xy';
      default: return 'animate-gradient-x'; // horizontal by default
    }
  })();
    ...
    >
      ...
      <div className="flex flex-col justify-center w-auto relative">
        <div className="flex flex-row p-0 m-0 mb-1 justify-between items-start">
          <h2
            className={`text-sm font-medium leading-snug mt-2 ${hasAdjustment ? 'text-yellow-300' : 'text-white'}`}
            title={channel.title}
          >
            {channel.title.length > 20 ? channel.title.slice(0, 15) + '...' : channel.title}
          </h2>
          <button
            onClick={() => onRemove(channel.id)}
            className="text-transparent hover:text-transparent text-xs ml-2 mt-1"
            title=""
        ...
        <div className="w-[175px] h-[40px] overflow-hidden -mt-1">
          {viewMode === 'iframe' ? (
            <iframe
              ...
              width="800px"
              frameBorder="0"
              src={`https://socialcounts.org/youtube-live-subscriber-count/${encodeURIComponent(channel.channelId)}/embed`}
              allowFullScreen
            ></iframe>
          ) : (
            <OdometerLiveCounter value={liveCount ?? 0} source={freshness ?? 'cached'} speed={30} />
          )}
        </div>
...
export function CardGrid({
      ...
      const workers = Array.from({ length: maxConcurrent }).map(async () => {
        while (queue.length > 0 && !isCancelled) {
          ...
          try {
            ...
            const res = await fetch(`${BASE}/api/subs/${channel.channelId}?bypassCache=${bypassCache}&cacheTTL=${cacheTTL}`);
            ...
            freshness[channel.channelId] = data.fromCache ? 'cached' : 'live';
            ...
            console.error(`❌ Failed to fetch for ${channel.channelId}`, err.message);
  ...
  }, [channels, pollInterval, bypassCache, cacheTTL, setLiveCounts, setFreshSources]);

  const sortedChannels = useMemo(() => {
    return [...channels].sort((a, b) => {
      const subA = (liveCounts[a.channelId] ?? 0) + (adjustments[a.channelId] ?? 0);
      const subB = (liveCounts[b.channelId] ?? 0) + (adjustments[b.channelId] ?? 0);
      return subB - subA;
    });
